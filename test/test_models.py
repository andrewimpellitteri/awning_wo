...
def test_create_user(app):
    ...
    with app.app_context():
        ...
        user = User(username='testuser', email='test@test.com', password_hash=password_hash, role='admin')
...
def test_user_default_role(app):
    ...
    """
    with app.app_context():
        user = User(username='testuser', email='test@test.com', password_hash='somehash')
        db.session.add(user)
        db.session.commit()
...
def test_customer_get_full_address(app):
    ...
    """
    with app.app_context():
        customer = Customer(Address='123 Main St', City='Anytown', State='CA', ZipCode='12345')
...
def test_customer_get_mailing_address(app):
    ...
    """
    with app.app_context():
        customer = Customer(MailAddress='P.O. Box 123', MailCity='Anytown', MailState='CA', MailZip='12345')
