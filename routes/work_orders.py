...
def list_work_orders():
    ...
    return render_template(
        "work_orders/list.html", work_orders=work_orders, status=status
    )
...
def new_work_order(customer_id=None):
    ...
    return render_template(
        "work_orders/form.html", customer=customer, customers=customers
    )
...
def create_work_order():
    ...
    work_order = WorkOrder(
        ...
        date_in=datetime.strptime(request.form.get("date_in"), "%Y-%m-%d").date()
        if request.form.get("date_in")
        else None,
        date_required=datetime.strptime(
            request.form.get("date_required"), "%Y-%m-%d"
        ).date()
        if request.form.get("date_required")
        else None,
