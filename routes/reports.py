...
def incomplete_orders():
    ...
    return render_template(
        "reports/incomplete.html", work_orders=incomplete_work_orders
    )
...
def completed_orders():
    ...
    if start_date:
        query = query.filter(
            WorkOrder.date_completed >= datetime.strptime(start_date, "%Y-%m-%d").date()
        )
    if end_date:
        query = query.filter(
            WorkOrder.date_completed <= datetime.strptime(end_date, "%Y-%m-%d").date()
        )

    completed_orders = query.all()

    return render_template(
        "reports/completed.html",
...
def summary_report():
    ...
    active_repair_orders = RepairOrder.query.filter(
        RepairOrder.date_completed.is_(None)
    ).count()
